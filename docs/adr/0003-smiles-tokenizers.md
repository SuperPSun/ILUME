...
AIS 固定 `atomInSmiles==1.0.2`；SPE 固定 `SmilesPE==0.0.3`；APE 固定上游 commit `ff1b3cc00476a8d017d7d54e925681a04475d47f`。APE 使用该 commit 的官方 pre-tokenization，再执行有界、确定性的相邻 pair merge 学习，避免上游小语料训练循环在词表预算不可达时无法终止。BPE 使用 Hugging Face `tokenizers` 的 `BpeTrainer`。
