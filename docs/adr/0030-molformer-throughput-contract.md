...
4. 科研训练合同固定为 batch 128、encoder/head learning rate `5e-6/5e-5`、最多50 epochs、patience 8，并在 train、checkpoint validation 和独立 valid/test 中启用 TF32。两组learning rate由batch 256合同按linear scaling减半；AdamW、weight decay、normalized MSE、5% warmup、cosine decay和validation MAE选择保持不变。
