...
> Patience、validation-best checkpoint 与训练预算由 [ADR-0045](0045-fixed-budget-baseline-training.md) 取代；吞吐和 scheduler 条款保持有效。
...
4. 科研训练合同固定为 batch 128、encoder/head learning rate `5e-6/5e-5`、最多10 epochs，并在 train、checkpoint validation 和独立 valid/test 中启用 TF32。两组learning rate由batch 256合同按linear scaling减半；AdamW、weight decay、normalized MSE、5% warmup和cosine decay保持不变。
