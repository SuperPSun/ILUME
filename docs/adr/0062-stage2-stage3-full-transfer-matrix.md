...
3. 每个 representation 分别训练 21 tasks × 5 folds 的独立
...
- 正式运行需要新生成10个encoder、10份representation bank和1050个MLP结果；现役
  Stage1 checkpoint、Stage2 prepared data/teacher cache和Stage3 prepared artifact可复用。
- baseline只训练一次，并由九个source共享引用。汇总前必须验证1050个job完整，以及同一
