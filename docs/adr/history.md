...
本页合并已完全退出的设计，只用于追溯理由，不提供运行命令或兼容入口。现役决定从 [ADR 索引](README.md) 查找；仍约束 legacy/Capacity 的 ADR 不在本次合并范围。

原始全文保存在 Git 提交 `1edf5d853bb736ffc59fe042ce17f54cf5e0a8a2`。需要精确复盘时，在仓库根运行 `git show 1edf5d8:docs/adr/<原文件名>`；原文件名见各条目。编号不重用。
