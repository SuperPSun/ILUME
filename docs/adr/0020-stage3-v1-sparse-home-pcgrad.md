...
>
> 2026-09-07：现役 v2 与对应 ADR-0034/0036 消融的 joint gradient clipping 和 virtual sampling 已由 [ADR-0046](0046-stage3-ownership-clipping-capped-sampling.md) 修订；legacy v1 与 Capacity v1 保留本文合同。
...
- 多 fold 调度的请求顺序、最大并发数和设备分配只属于运行参数；每个 fold 继续持有独立 run directory、identity、日志、checkpoint 和恢复状态。
- 正式 prepare、100 epoch 训练与五折评估必须由用户显式执行；仓库验收只使用临时小数据与短训练。运行产物状态属于 README/metadata 的操作事实，不改变本 ADR 的科学合同。
