...
corpus artifact 固定为 `kind="ilume_stage1_corpus"`、`format_version=2`。shard 只保存训练消费的 tensor 与 sample/role id，fingerprint 以 `uint8` 保存并在 collate 时恢复为 float32；provenance 继续位于 manifest 和 audit。v1 及更早 corpus 一律拒绝并要求重新 prepare，Stage 1 checkpoint 仍为 `kind="ilume_stage1_pretraining"`、`format_version=1`。
