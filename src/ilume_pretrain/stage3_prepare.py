...
from .data import ROLE_TO_ID, _build_sample, _inspect_entity_qc
from .descriptors import calculate_descriptors, rdkit_descriptor_names
from .masking import MultimodalPacker
from .progress import ProgressReporter
from .stage2_config import stage2_config_from_dict
from .stage2_data import _load_pretrain_inputs
from .stage2_model import Stage2AlignmentModel, load_stage1_model, sha256_file
...
def load_frozen_stage2(
    ...
    *,
    ...
    if not isinstance(raw_config, dict) or checkpoint.get("config_hash") != _config_hash(raw_config):
        raise ValueError("Stage 2 checkpoint configuration hash mismatch")
    stage2_config = stage2_config_from_dict(raw_config)
    data_metadata = stage2_config.data.artifacts_dir / "metadata.json"
    if checkpoint.get("data_metadata_hash") != sha256_file(data_metadata):
        raise ValueError("Stage 2 checkpoint data artifact hash mismatch")
    if checkpoint.get("pretrain_checkpoint_hash") != sha256_file(stage2_config.initialization.checkpoint):
        raise ValueError("Stage 2 checkpoint Stage 1 source hash mismatch")
    ...
    provenance = {
        ...
        "stage2_checkpoint_hash": checkpoint_hash,
        "stage2_config_hash": checkpoint["config_hash"],
        "stage2_data_metadata_hash": checkpoint["data_metadata_hash"],
        "stage1_checkpoint_hash": checkpoint["pretrain_checkpoint_hash"],
    ...
    del checkpoint
    return model, provenance, loaded.checkpoint_hash
...
def _stage2_pretrain_inputs(raw_stage2_config: dict[str, Any]):
    config = stage2_config_from_dict(raw_stage2_config)
...
def load_frozen_embeddings(
    ...
    if metadata.get("provenance", {}).get("stage2_checkpoint_hash") != sha256_file(config.initialization.stage2_checkpoint):
        raise ValueError("Stage 3 frozen cache Stage 2 checkpoint hash mismatch")
