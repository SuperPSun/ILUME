...
def run_stage3_training(
    ...
    *,
    ...
    if resume_from is not None:
        checkpoint = _load_training_checkpoint(resume_from)
        if checkpoint.get("fold") != fold:
            raise ValueError("Stage 3 checkpoint fold does not match")
        checkpoint_config = _stage3_config_from_checkpoint_dict(checkpoint["config"])
    ...
    with reporter.bar(
        ...
        while any(not runtime.stopped for runtime in runtimes.values()):
            ...
            for domain in config.active_domains:
                ...
                progress.update(1)
            interval = config.training.save_every_n_cycles
            all_stopped = all(runtime.stopped for runtime in runtimes.values())
            should_checkpoint = (
                interval not in {None, 0} and cycle % interval == 0
            ) or all_stopped
