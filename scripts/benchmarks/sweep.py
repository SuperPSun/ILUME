...
def _aggregate(root: Path, config: Any) -> dict[str, Any]:
    ...
    for task in configured_tasks(config, "stage3"):
        ...
        for fold in config.stage3.folds:
            run = _latest_completed(
                task_root / f"evaluate_valid_fold{fold}", "summary.json"
            )
            if run:
                payload = json.loads(
        ...
        if len(fold_values) == len(config.stage3.folds):
            ...
            stage3_valid[task] = {
                metric: mean_sample_std(
                    [
                        float(row["targets"][target][metric])
        ...
        if run:
            ...
            stage3_test[task] = next(
                iter(payload["ensemble"]["targets"].values())
            )
            ...
            for destination, values in (
                ...
                for key, value in values.items():
                    if key in destination and destination[key] != value:
                        raise ValueError(
                            f"Conflicting reporting comparison item: {key}"
                        )
    ...
    study_ids = {
        item["study_id"]
        for item in [*stage3_valid_reporting, *stage3_test_reporting]
    }
    ...
    stage3_sections = {
        "stage3_test": {
            ...
            "comparison_identity": merged_comparison(
                [
                    item
                    for item in stage3_test_reporting
