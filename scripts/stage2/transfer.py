...
def _worker(
    ...
    if (root.parent.parent / "resolved_sampling_plan.json").exists():
        raise ValueError("Balanced transfer artifacts are retired and cannot be resumed")
    if root.exists():
...
def main() -> None:
    ...
    if (root / "resolved_sampling_plan.json").exists():
        raise ValueError("Balanced transfer artifacts are retired and cannot be resumed")
    ...
    try:
        if baseline_root.exists():
            ...
            require_full_transfer_artifact(baseline)
        else:
