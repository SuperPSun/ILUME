...
- 科研参数与 prepare 执行参数都写入完整自包含 YAML；`preparation` 不进入实验身份。正式 v1 配置位于 `configs/v1`；预注册实验配置位于 `configs/experiments_v1/<stage>`；内部消融配置位于 `configs/ablations`。
