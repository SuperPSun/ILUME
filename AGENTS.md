...
Stage 3 现役合同以 ADR-0020、ADR-0039、ADR-0046、ADR-0047 和 `configs/v2/stage3/base.yaml` 为准、identity/audit 边界以 ADR-0021 为准：1024 维冻结 Stage 2 Object v3 表示、21 个 sparse-label observation task、6 个 meta-group、动态 HoME、raw sampling 与 ownership-aware clipping；训练固定为 5-epoch joint bootstrap、10-epoch shared consolidation、六个从同一 Phase-B state 分叉并 stitch 的 group specialization，以及 21 个从同一 Phase-C stitched state 分叉并 stitch 的 task specialization。A/B 使用 hierarchical PCGrad，C 只在组内 GROUP block 使用 PCGrad，D 关闭 PCGrad；所有 scope 使用固定最后 epoch，validation 仅记录。GROUP capacity、PRIVATE LR scale 与 C/D epoch 由配置逐 group/task 固定，冻结必须使用 `requires_grad=False`。Base 的 training/validation microbatch 上限为 1024 且属于严格 training identity。legacy v1 与 Capacity v1 继续由 ADR-0027 约束，保留整模 clipping、`max(N_t,1000)` virtual oversampling、80/20 refinement 与 `taskwise_refined`。ADR-0034 的隔离消融只以 RDKit 2D + 两个 GLOBAL Linear→LayerNorm adapter 替换 frozen Object representation，其他现役 Stage3 数值合同保持不变；不得加载 Stage1/2 checkpoint、使用 plugin、增加 HPO 或与 Object artifact/checkpoint 交叉恢复。ADR-0036 的 No-Stage1 隔离消融以共享 RDKit-217 MLP 替换 Stage1 backbone，保留 Stage2 ObjectEncoder 与 Stage3 Base；Stage2 仅训练八个 object/interaction task，禁止 Stage1/teacher artifact，且不得与现役 Stage2 artifact/checkpoint 交叉加载。ADR-0010～0012 仅为历史。
