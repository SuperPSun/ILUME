...
- 周期 checkpoint 全部保留。Stage 1 以 `last.pt` 表示最新完整恢复状态。Stage 2 保存不可覆盖的 `checkpoint_epoch_XXXXX.pt`，现役 v2 的最终 joint checkpoint 直接导出 `stage2_encoder.pt`；Stage 3 按 YAML interval 保存不可覆盖的完整 epoch checkpoint。普通 final checkpoint 都表示真实历史恢复状态，不生成 `best.pt` 或 `last.pt`。Stage 3 的最终评估模型是独立的 `taskwise_refined.pt` 与 manifest，不得伪装成某个 epoch checkpoint。
